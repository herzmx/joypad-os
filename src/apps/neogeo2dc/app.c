// app.c - NEOGEO2DC App Entry Point
// NEOGEO native controller input to DC gamepad output adapter
//
// This app polls native NEOGEO controllers and routes input to DC Gamepad output.
...
void app_init(void)
{
    printf("[app:neogeo2usb] Initializing NEOGEO2DC v%s\n", APP_VERSION);

    // Initialize Arcade host driver input
    arcade_host_init_pins(&arcade_config[0]);
    // Initialize Dreamcast maple pins
    setup_maple_gpio(DC_MAPLE_PIN1, DC_MAPLE_PIN5);

    // Configure router for NEOGEO → DC routing
    ...
    // Add route: Native NEOGEO → DC Gamepad
    router_add_route(INPUT_SOURCE_NATIVE_ARCADE, OUTPUT_TARGET_DREAMCAST, 0);
    ...
    printf("[app:neogeo2usb]   Routing: NEOGEO → DC Gamepad\n");
    ...
    printf("[app:neogeo2usb]   Dreamcast Maple pins: GPIO%d, GPIO%d\n",
